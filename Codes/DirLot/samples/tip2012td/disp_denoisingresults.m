...
cnames = { 'DB5' 'SON4' 'GLN2' 'NSCT' 'UDN4' };
wlist = { 'db5' 'son4' 'glt' 'nsct' 'udn4' };
...
nImgs = length(imgset);
for idx = 1:nImgs
    load(sprintf('./results/wv_psnrs_%s.mat',imgset{idx}))
    ...
    for iSigma = 1:nSigmas
        ...
        for iTrx = 1:length(wlist)
            if strcmp(wlist{iTrx},'nsct')
                load(sprintf('./results/nsct_psnrs_%s.mat',imgset{idx}))
                itype = 1;
            elseif strcmp(wlist{iTrx},'glt')
                load(sprintf('./results/genlot_psnrs_%s.mat',imgset{idx}))
                itype = 1;
            else
                load(sprintf('./results/wv_psnrs_%s.mat',imgset{idx}))
...
%% Display graph
crit = { 'MSE' 'PSNR' 'SSIM' };
lcol = { [1 0 0] [1 1 0] [0 1 0] [0 0 1] [0 0 0]}; 
lmrk = { 'o' '.' 'x' '+' '*' };
icrit = 1;
for itab = [ tmse tpsnr tssim ];
    dat = get(itab,'Data');
    ...
    for iImg = 1:nImgs
        ...
        legend(cnames{1},cnames{2},cnames{3},cnames{4},cnames{5},'Location','Best')
        iRow = jRow;
    end
    icrit = icrit + 1;
end
