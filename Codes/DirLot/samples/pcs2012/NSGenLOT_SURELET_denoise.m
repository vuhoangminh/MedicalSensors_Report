function output = NSGenLOT_SURELET_denoise(input,wtype,sigma)
%NSGENLOT_SURELET_DENOISE Removes additive Gaussian white noise using the
...
%------------------------------
fdirlot = ForwardDirLot([dec dec],paramMtx,'termination');
WT = cell(nBands,1);
LL = cell(nLevels,1);
nOrientations = 3;
coefLL = input;
...
for iLevel = 1:nLevels
    ...
    for iOrientation=1:nOrientations
        iSubband = idx+iOrientation;
        Y = WT{iSubband};
        Yp = abs(imfilter(LL{iLevel},w{iOrientation},'conv','symmetric'));
...
%% Inverse transform
%------------------
idirlot = InverseDirLot([dec dec],paramMtx,'termination');
