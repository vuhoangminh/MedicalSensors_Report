function [corner_index,corner_index3,corner_index4,marked_img,marked_img3,marked_img4,edge_map,thresh]=...
    curvature_corner_new_marker(varargin)
...
[BW, thresh]=edge(I,'canny');  % Detect edges
...
%tic
[corner_index,corner_index3,corner_index4]=get_corner(curve,curve_start,curve_end,curve_mode,curve_num,BW,sig,Endpoint,C,T_angle); % Detect corners
...
img=marker(img,corner_index,'Circle','White');
...
% edge_map= bwareaopen(BW_edge,Open_size);
edge_map=BW_edge;
%figure(2)
%imshow(~BW_edge)
%title('Edge map')
...
% final result  
        ...
        for j=1:n     
            ...
            corner_index(corner_num,:)=curve{i}(extremum(j),:);
        end
...
function img1=marker(img,corner_index,Shape,BorderColor)
markerInserter = vision.MarkerInserter('Shape',Shape,'BorderColor',BorderColor);
NumCorner=size(corner_index,1);
icorner_index=zeros(NumCorner,2);
icorner_index(:,1)=corner_index(:,2);
icorner_index(:,2)=corner_index(:,1);
img1 = step(markerInserter, img, icorner_index);
