function [corner_index,corner_index3,corner_index4,marked_img,marked_img3,marked_img4,edge_map]...
    =curvature_corner_frame2(varargin)

%   CORNER Find corners in intensity image. 
%   
%       CORNER works by the following steps:
...
%
%       Syntax :    
%       [corner_index,marked_img,edge_map]=corner(I,C,T_angle,sig,H,L,Endpiont,Gap_size)
%
%       Input :
...
%
%       Output :
%       corner_index -  a position pair list of detected corners in the input image.
...
%       corner_index = corner(I,[],[],[],0.2);
%       [corner_index, marked_image] = corner;
%       corner_index = corner([],1.6,155);
%
%
...
[I,C,T_angle,sig,H,L,Endpoint,Gap_size,Open_size] = parse_inputs(varargin{:});
...
%tic
[curve,curve_start,curve_end,curve_mode,curve_num,edge_map]=extract_curve(BW,Gap_size,Open_size);  % Extract curves
%time_for_extracting_curve=toc

%tic
[corner_index,corner_index3,corner_index4]=get_corner(curve,curve_start,curve_end,...
    curve_mode,curve_num,BW,sig,Endpoint,C,T_angle); % Detect corners
...
for i=1:size(corner_index,1)
    img=mark(img,corner_index(i,1),corner_index(i,2),5);
end

for i=1:size(corner_index3,1)
    img3=mark(img3,corner_index3(i,1),corner_index3(i,2),5);
end

for i=1:size(corner_index4,1)
    img4=mark(img4,corner_index4(i,1),corner_index4(i,2),5);
end
...
for i=1:1:60
    img5=mark(img5,sorted(i,1),sorted(i,2),5);
end
%add

sorted_img=img5;
imshow(sorted_img);
title('10 strongest corners');
...
function [curve,curve_start,curve_end,curve_mode,cur_num,edge_map]=extract_curve(BW,Gap_size,Open_size)
...
function [corner_index,corner_index3,corner_index4]=get_corner(curve,curve_start,curve_end,curve_mode,curve_num,BW,sig,Endpoint,C,T_angle)

corner_num=0;
corner_num3=0;
corner_num4=0;
corner_index=[]; %final corners
corner_index3=[]; %corners after step 3
corner_index4=[]; %corners after step 4
...
for i=1:curve_num;
    ...
    if L>W
        ...
        %add
    	size_ROS=zeros(size(extremum));
        mean_ROS=zeros(size(extremum));
        %add        
        ...
        for j=2:2:n
            ...
            %add
%             size_ROS(j)=size(ROS,1);
            size_ROS(j)=size(curve{i},1);
            mean_ROS(j)=K(extremum(j))-mean(ROS);
            %add
        ...
        %add
        size_ROS=size_ROS(2:2:n);
        mean_ROS=mean_ROS(2:2:n);
        %add
        ...
        %add
        size_ROS=size_ROS(find(flag==1));
        mean_ROS=mean_ROS(find(flag==1));
        %add        
        
		extremum4=extremum; %after step 4
        ...
        while sum(flag==0)>0
            ...
            if size(extremum,2)==0
                ...
                size_ROS=size_ROS(find(flag~=0));
                mean_ROS=mean_ROS(find(flag~=0));
            end
        end
...
% final result  
        ...
        size_ROS=size_ROS(find(extremum>0 & extremum<=L));
        mean_ROS=mean_ROS(find(extremum>0 & extremum<=L));
        
        n=size(extremum,2);     
        for j=1:n     
            corner_num=corner_num+1;
            corner_index(corner_num,1)=curve{i}(extremum(j),1);
            corner_index(corner_num,2)=curve{i}(extremum(j),2);
%             corner_index(corner_num,3)=size_ROS(j)*mean_ROS(j);
%             corner_index(corner_num,3)=mean_ROS(j);
%             if j==1
%                 corner_index(corner_num,3)=mean_ROS(j)*extremum(j);
%             else
%                 corner_index(corner_num,3)=mean_ROS(j)*(extremum(j)-extremum(j-1));
%             end

            corner_index(corner_num,3)=mean_ROS(j);
            corner_index(corner_num,4)=size_ROS(j);
        end
		
% after step 3
		extremum3=extremum3-W;
        extremum3=extremum3(find(extremum3>0 & extremum3<=L));
		n3=size(extremum3,2); 
        for j=1:n3     
            corner_num3=corner_num3+1;
            corner_index3(corner_num3,:)=curve{i}(extremum3(j),:);
        end
		
% after step 4
		extremum4=extremum4-W;
        extremum4=extremum4(find(extremum4>0 & extremum4<=L));
		n4=size(extremum4,2);  	
        for j=1:n4
            corner_num4=corner_num4+1;
            corner_index4(corner_num4,:)=curve{i}(extremum4(j),:);
        end
    end
end
...
if Endpoint
    for i=1:curve_num
        if size(curve{i},1)>0 & curve_mode(i,:)=='line'
            ...
            compare_corner=corner_index-ones(size(corner_index,1),1)*curve_start(i,:);
            ...
            if min(compare_corner)>25       % Add end points far from detected corners 
                corner_num=corner_num+1;
                corner_index(corner_num,:)=curve_start(i,:);
            end
            
            % End point compare with detected corners
            compare_corner=corner_index-ones(size(corner_index,1),1)*curve_end(i,:);
            compare_corner=compare_corner.^2;
            compare_corner=compare_corner(:,1)+compare_corner(:,2);
            if min(compare_corner)>25
                corner_num=corner_num+1;
                corner_index(corner_num,:)=curve_end(i,:);
            end
        end
    end
end
...
function [I,C,T_angle,sig,H,L,Endpoint,Gap_size,Open_size] = parse_inputs(varargin)

narginchk(0,9);
Para=[1.5,162,3,0.35,0,0,1,50]; %Default experience value;
