...
im1 = dicomread('C:\Users\minhm\Desktop\Medical Sensors code\dataset\PET_image');

im1 = double(im1);
im1 = imresize(im1, 256/144);
% im2 = histeq(im1);
im = im1 / max(max(im1));
...
figure;
subplot(2,2,1), imagesc(im(150:164, 105:117), range); axis image off
set(gca, 'FontSize', 8);
title('Original Image', 'FontSize', 10);

subplot(2,2,2), imagesc(nim(150:164, 105:117), range); axis image off
set(gca, 'FontSize', 8);
title(sprintf('Noisy Image (SNR = %.2f dB)', ...
              SNR(im, nim)), 'FontSize', 10);

subplot(2,2,3), imagesc(wim(150:164, 105:117), range); axis image off
set(gca, 'FontSize', 8);
title(sprintf('Denoise using Wavelets (SNR = %.2f dB)', ...
              SNR(im, wim)), 'FontSize', 10);

subplot(2,2,4), imagesc(cim(150:164, 105:117), range); axis image off
set(gca, 'FontSize', 8);
title(sprintf('Denoise using Contourlets (SNR = %.2f dB)', ...
              SNR(im, cim)), 'FontSize', 10);
